 ...
 */
public abstract class NettyMqttServer extends BaseService implements Server {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());

    public enum State {Created, Initialized, Starting, Started, Shutdown}

    protected final AtomicReference<NettyMqttServer.State> serverState = new AtomicReference<>(NettyMqttServer.State.Created);
    ...
    public void init() {
        if (!serverState.compareAndSet(NettyMqttServer.State.Created, NettyMqttServer.State.Initialized)) {
    ...
    public boolean isRunning() {
        return serverState.get() == NettyMqttServer.State.Started;
    }

    @Override
    public void stop(Listener listener) {
        if (!serverState.compareAndSet(NettyMqttServer.State.Started, NettyMqttServer.State.Shutdown)) {
    ...
    public void start(final Listener listener) {
        if (!serverState.compareAndSet(NettyMqttServer.State.Initialized, NettyMqttServer.State.Starting)) {
    ...
    private void createServer(Listener listener, EventLoopGroup boss, EventLoopGroup work, ChannelFactory<? extends ServerChannel> channelFactory) {
        ...
        try {
            ...
            b.bind(port).addListener(future -> {
                if (future.isSuccess()) {
                    serverState.set(NettyMqttServer.State.Started);
    ...
    protected ChannelHandler getEncoder() {
        return MqttEncoder.INSTANCE;//每连上一个链接调用一次, 所有用单利
    }
